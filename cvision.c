...
int vc_gray_lowpass_gaussian_filter(IVC *src, IVC *dst)
{
	...
	int x, y, kx, ky, kernel_size = 5, pos, posk;
	double sigma = 1.0, gauss[5] = {0.061, 0.242, 0.383, 0.242, 0.061}, gausssum;
		...
		for (x = 0; x < width; x++)
		{
			...
			for (ky = 0; ky <= kernel_size; ++ky)
				for (kx = 0; kx <= kernel_size; ++kx){
					...
					{
						pos = (y + ky) * bytesperline + (x + kx);
						gausssum += (datasrc[pos]) * gauss[kx] * gauss[ky];
					}
				}

			posk = y * bytesperline + x;
			datadst[posk] = (unsigned char)gausssum;
		}
	}
...
int vc_gray_lowpass_mean_filter(IVC *src, IVC *dst, int kernelsize)
{
	...
	int x, y, ky, kx, pos, posk, masksize, average;
	...
	// Mask size
	masksize = kernelsize * kernelsize;
	...
	for (y = 0; y < height; y++)
	{
		for (x = 0; x < width; x++)
		{
			...
			// NxM neighbors
			for (average = 0, ky = -offset; ky <= offset; ++ky)
				for (kx = -offset; kx <= offset; ++kx)
					if ((y + ky >= 0) && (y + ky < height) && (x + kx >= 0) && (x + kx < width))
					{
						posk = (y + ky) * bytesperline + (x + kx);
						average += datasrc[posk];
					}

			average /= (float)masksize;
			datadst[pos] = average;
		}
	}
...
int vc_gray_lowpass_median_filter(IVC *src, IVC *dst, int kernelsize)
{
	...
	int width = src->width;
	int height = src->height;
	int offset = (kernelsize / 2), kernelsizepair = (((kernelsize * kernelsize) + 1) / 2);
	int x, y, ky, kx, pos, posk, i, j, qty, *maskdata = NULL;
	...
	for (y = 0; y < height; y++)
	{
		for (x = 0; x < width; x++)
		{
			pos = y * bytesperline + x;
			qty = 0;
			maskdata = malloc(sizeof(int)*kernelsize * kernelsize);
			// NxM neighbors
			for (ky = -offset; ky <= offset; ++ky)
				for (kx = -offset; kx <= offset; ++kx)
					...
					{
						posk = (y + ky) * bytesperline + (x + kx);
						maskdata[qty] = datasrc[posk];
						qty++;
					}

			if (qty != (kernelsize * kernelsize))
				kernelsizepair = ((qty + 1) / 2);

			// Sort maskdata[] array by ascendent order
			for (i = 0; i < qty - 1; i++)
			{
				for (j = i + 1; j < qty; j++)
				{
					if (maskdata[j] < maskdata[i])
					{
						int tmp = maskdata[i];
			...
			datadst[pos] = maskdata[kernelsizepair];
			free(maskdata);
		}
	}
...
IVC *vc_image_new(int width, int height, int channels, int levels)
{
	...
	if (image == NULL)
		return NULL;
	if ((levels <= 0) || (levels > SIZEOFUCHAR))
		return NULL;
